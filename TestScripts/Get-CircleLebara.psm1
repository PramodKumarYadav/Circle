function Get-CircleLebara{
    ...
    Process{
        ...
        foreach($PDFFile in $PDFFiles){
            ...
            $RegExDataToFilter = '[0-9][0-9]:[0-9][0-9]:[0-9][0-9].*€.*'
            ...
            $CommaFormatTXTFile = "$TestResultsDir\CommaFormatTXTFile.txt"
            $newcontent = Convert-FixedLengthLebaraRecordsToCSV -Path "$FilteredTXTFile" -Positions @(11, 17, 26) -Delimiter ',' # fixed positions in lebara mobile
            $newcontent > $CommaFormatTXTFile

            # Convert this TXT file to a proper CSV file
            $FilteredCSVFile = "$TestResultsDir\FilteredCSVFile.csv"
            $Header = 'Date','Time','CallDuration','Dialled number','Cost'
            Import-Csv "$CommaFormatTXTFile" -delimiter "," -Header $Header | Export-Csv "$FilteredCSVFile"
